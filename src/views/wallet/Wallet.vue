<template>
  <div class="shark-data-table">
    <div class="pt-32 mb-32">
      <span class="head-title">Wallets</span><br />
      <span class="subtitle">View all the information related to Wallets</span>
    </div>
    ...
    <div class="buttons-row" >
      <SToggle
        ...
        @selected="selectWalletType"
      />
      <SButton
        v-if="selectWallet === 'wallet'"
        title="Create Wallet"
        pill
        outline
        small
    ...
    <div v-if="selectWallet === 'archived'">
      <ArchivedWallet :configs="archivedConfigs"/>
    </div>
    
    <ArchiveDialog
      v-if="archiveDialog"
      @close="closeDialog"
      @archiveData="archiveWallet"
...
import ArchivedWallet from "./ArchivedWallet.vue";
...
export default {
  ...
  data() {
    return {
      ...
      parentSystemSettings: {
        ...
        selectType: "none",
        moreOptions: [],
        actionsOptions: ["View", "Edit", "Archive"],
        columnIcons: [{ field: "", icon: "" }],
        pillConfig: [
        ...
        user: JSON.parse(localStorage.getItem("currentUserDetails")).profile
          .username,
        uppercaseColumns: ["Capacity Unit"], // pass column name whose data should be shown in caps
  ...
  methods: {
    ...
    selectWalletType(obj) {
      this.selectWallet = obj.value;
    },
    ...
    getActionsOption(val) {
      ...
      if (val.value == "Archive") {
        ...
        this.walletUUID = val.row.walletUUID;
      }
    },
    ...
    getPaginatedWallet(filters, sortField, sortDirection, offset, limit,searchQuery) {
      ...
      AssetManagementService.getPaginatedWalletForSDT(requestObj)
        .then((response) => {
    ...
    archiveWallet() {
      AssetManagementService.archiveOrDeleteWalletByUUID(
        this.walletUUID,
        "archive"
      )
        .then((res) => {
  ...
  mounted() {
    ...
    assetPersonnelService
      ...
      .then((response) => {
        ...
        allConfigs = allConfigs.filter(
          (data) => data.pageName === "WalletManagement"
        )[0];

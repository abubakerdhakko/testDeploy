<template>
  <div>
    ...
    />
    <DeleteDialog
      v-if="deleteAchivedTemp"
      @close="closeArchDelDialog"
...
export default {
  ...
  methods: {
    ...
    closeArchDelDialog() {
      this.deleteAchivedTemp = false;
    },

    restoreArchived() {
      this.restoreAssetDialog = false;
      this.loaderFlag = true;
      AssetManagementService.archiveOrDeleteAssetCateTemplate(
        this.assetId,
        "restore"
      )
        .then((response) => {
          if (response.status === 200) {
            ...
            this.showToast("Template Asset Restored Successfully", "success");
            this.restoreAssetDialog = false;
            this.loaderFlag = true;
          }
        })
    ...
    deleteArchivedDialog() {
      ...
      this.restoreAssetDialog = false;
      this.loaderFlag = true;
      AssetManagementService.DeleteArchivedAssetCateTemplate(
        this.assetId,
        "Delete"
      )
        .then((response) => {

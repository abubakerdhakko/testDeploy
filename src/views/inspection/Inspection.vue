<template>
  <div class="shark-data-table">
    <div class="pt-32 mb-32">
      <span class="head-title">Inspections</span><br />
      <span class="subtitle"
        >View all the information related to inspections</span
      >
    </div>
    <SToggle
    ...
    />
    <div v-if="selectedTemplate === 'submitted'">
      <Sdt
    ...
    <div v-if="selectedTemplate === 'pending'">
      <PendingInspection
        @showToast="showToast"
        :configs="pendingInspectionConfigs"
      />
    </div>
    <div v-if="selectedTemplate === 'archived'">
      <ArchivedInspection
        @showToast="showToast"
        :configs="archivedInspectionConfigs"
      />
    </div>
    <DeleteDialog
...
import PendingInspection from "../../views/inspection/pendingInspection.vue";
import ArchivedInspection from "../../views/inspection/ArchivedInspections.vue";
...
export default {
  ...
  data() {
    return {
      ...
      parentSystemSettings: {
        ...
        selectType: "none",
        moreOptions: [],
        actionsOptions: ["View", "Delete", "Archive"],
        columnIcons: [],
        pillConfig: [
      ...
      inspections: [],
      sdtData: "",
      pendingInspectionConfigs: "",
      archivedInspectionConfigs: "",
      fieldNames: [],
    };
  },
  ...
  methods: {
    ...
    deleteDialog() {
      ...
      InspectionService.deleteInspectionByUUID(this.inspectionID, "delete")
        .then((response) => {
          if (response.status === 200) {
            this.loaderFlag = false;
            this.showToast("Inspection Deleted Successfully", "success");
            this.deleteInspectionDialog = false;
    ...
    getActionsOption(val) {
      ...
      //     params: {
      //       inspectionUuid: val.row.uuid,
      //       inspectionNumber: val.row.issueNumber,
      //       templateType: "editIssue",
    ...
    setTableConfigs() {
      ...
      // console.log('this.tableConfig.columns;',this.tableConfig)
    },
    ...
    getPaginatedInspections(
      ...
      inspectionService
        .getPaginatedInspectionReportsForSDT(requestObj)
        .then((response) => {
          if (response.status == 200) {
            response.data.sdtData.content.forEach((item) => {
            ...
            let inspections = response.data.sdtData.content;
            response.data.sdtData.offset = offset + 1;
            this.loadSDT(response, inspections);
            this.loaderFlag = false;
          } else {
            this.loading = false;
    ...
    selectedTemplate: function(newVal, oldVal) {
      if (newVal === "pending") {
        ...
        allConfigs = allConfigs.filter(
          (data) => data.pageName === "pendingInspectionTemplate"
        )[0];
        this.pendingInspectionConfigs = allConfigs;
        // console.log('this.pendingInspectionConfigs=>',this.pendingInspectionConfigs)
      } else if (newVal === "archived") {
        let allConfigs = JSON.parse(this.orgConfigsResponse.data.tableConfigs);
        allConfigs = allConfigs.filter(
          (data) => data.pageName === "archivedInspections"
        )[0];
        this.archivedInspectionConfigs = allConfigs;
      }
    },
  },

<template>
  <div class="shark-data-table">
    <div class="pt-32">
      <span class="head-title">Consumptions</span><br />
      <span class="subtitle"
        >View all the information related to consumptions</span
      >
    </div>
...
export default {
  ...
  components: { AddConsumption, loader },
    ...
    return {
      ...
      parentSystemSettings: {
        ...
        user: JSON.parse(localStorage.getItem("currentUserDetails")).profile
          .username,
        uppercaseColumns: ["Currency"], // pass column name whose data should be shown in caps
  ...
  methods: {
    ...
    addConsumption() {
      ...
      this.formTitle = "Add Consumption";
    },
    ...
    getActionsOption(val) {
      if (val.value == "View") {
        this.comsumptionId = val.row.id;
        this.formTitle = "View Consumption";
        this.readyOnly = true;
        this.viewConsumptionDialog = true;
      }
      if (val.value == "Edit") {
        this.comsumptionId = val.row.id;
        this.formTitle = "Edit Consumption";
    ...
    getPaginatedConsumptions(
      ...
      assetManagementService
        .getPaginatedConsumptionsForSDT(requestObj)
        .then((response) => {
          if (response.status == 200) {
            response.data.sdtData.content.map((item) => {
              ...
              if (item.meterType === "1") {
                item.meterType = "Primary";
                item.usageUnit = item.primaryUsageUnit;
              } else if (item.meterType === "2") {
                item.meterType = "Secondary";
                item.usageUnit = item.secondaryUsageUnit;
              }
            });
    ...
    assetPersonnelService
      ...
      .then((response) => {
        ...
        this.tableConfig = allConfigs.filter(
          (data) => data.pageName === "Consumptions"
        )[0];

<template>
  <div>
    <div class="pt-32">
      <span class="head-title">Last Import Records</span><br />
      <span class="subtitle">View information related to Asset Imports</span>
    </div>
  <div class="importRec">
    <Sdt
  ...
  methods: {
    ...
    getActionsOption(val) {
      if (val.value == "View") {
        ...
        this.downloadAssetTemplate(val.row.uuid);
      }
    },

    downloadAssetTemplate(uuid) {
      assetManagementService
        .downloadFailureImports(uuid)
        .then((res) => {
    ...
    loadTable(sdtData) {
      ...
      this.getPaginatedLastAssetImportsForSDT(
        filters,
    ...
    getPaginatedLastAssetImportsForSDT(
      filters,
      sortField,
      sortDirection,
      offset,
      ...
      assetManagementService
        .getPaginatedLastAssetImportsForSDT(requestObj)
        .then((response) => {
          if (response.status == 200) {
            response.data.sdtData.content.map((item) => {
              item.importDate = moment.utc(item.importDate).local().format();
            });
            ...
            this.toastMessage =
              "An Unexpected Error occurred while getting last Asset imports.";
            this.toastType = "error";
            this.loaderFlag = false;
            this.toastFlag++;
          }
    ...
    assetPersonnelService
      ...
      .then((response) => {
        ...
        this.tableConfig = allConfigs.filter(
          (data) => data.pageName === "lastImportRecords"
        )[0];
      ...
      .catch((error) => {
        if (error) {
          this.toastMessage =
            "An Unexpected Error occurred while Error fetching configs.";
          this.toastType = "error";
          this.loaderFlag = false;
          this.toastFlag++;
        }

<template>
  <div>
    <div class="pt-32">
      <span class="head-title">Import Asset Templates</span><br />
      <span class="subtitle"
        >View all the information related to Existing Templates</span
      >
    </div>
...
export default {
  ...
  components: { CreateImportTemplate, loader },
  ...
  methods: {
    ...
    getActionsOption(val) {
      if (val.value == "View") {
        this.templateData = val.row;
        this.viewTemplate();
      } else if (val.value == "Download") {
        this.loaderFlag = true;
        this.createDownloadTemplate(JSON.parse(val.row.csvColumnData));
        let download = {
    ...
    createDownloadTemplate(csvColumnData) {
      this.downloadTemplate = "";
      let i = 0;
      csvColumnData.map((data) => {
        this.downloadTemplate =
          this.downloadTemplate +
          data.name +
          (csvColumnData[i + 1] !== undefined ? "," : "");
        i += 1;
      });
    },
    ...
    downloadAssetTemplate(downloadRequest) {
      assetManagementService
        .downloadAssetImportTemplate(downloadRequest)
        .then((res) => {
    ...
    loadTable(sdtData) {
      ...
      this.getPaginatedAssetImportTemplatesForSDT(
        filters,
    ...
    loadSDT(response, data) {
      ...
      // this.page = response.data.sdtData.offset;
      console.log(data);
      let eventData = {
    ...
    getPaginatedAssetImportTemplatesForSDT(
      filters,
      sortField,
      sortDirection,
      offset,
      ...
      assetManagementService
        .getPaginatedAssetImportTemplatesForSDT(requestObj)
        .then((response) => {
          if (response.status == 200) {
            response.data.sdtData.content.map((item) => {
              item.createdDate = moment.utc(item.createdDate).local().format();
            });

            let templates = response.data.sdtData.content;
            response.data.sdtData.offset = offset + 1;
            this.loadSDT(response, templates);
            this.loaderFlag = false;
          } else {
            this.toastMessage =
              "An Unexpected Error occurred while getting import Asset templates.";
            this.toastType = "error";
            this.loaderFlag = false;
            this.toastFlag++;
          }
    ...
    assetPersonnelService
      ...
      .then((response) => {
        ...
        this.tableConfig = allConfigs.filter(
          (data) => data.pageName === "ExistingAssetCSVTemplates"
        )[0];

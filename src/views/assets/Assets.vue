<template>
  <div class="shark-data-table pt-32 pb-80">
    <div style="margin-bottom: 40px">
      <span class="head-title">Assets</span><br />
      <span class="subtitle">View all the information related to assets</span>
    </div>
    ...
    <div class="buttons-row">
      ...
      />
      <div class="assets-btn" v-if="assetOption === 'asset'">
        <div class="dropdown">
          <div>
            <button
              class="dropbtn smallButton pillDesign outlineDesign hoverButton"
            >
              Import Assets
              <img
                src="assets/icons/grey-arrow.png"
                width="8px"
                height="4px"
                class="import-asset"
              />
            ...
            <div class="dropdown-content">
              <a id="add" @click="() => importOption('add')">Add & Update</a>
              <!-- <a id="update" @click="() => importOption('update')">Update</a> -->
              <a id="history" @click="() => importOption('history')"
                >Show History</a
              >
              <a id="existingTemplates" @click="() => importOption('existing')"
    ...
    <div v-if="assetOption === 'archivedAssets'">
      <ArchivedAssets :configs="archivedAssetsConfigs" />
    </div>
...
import AssetImportDialog from "../assets/components/AssetImportDialog.vue";
import ExportAssetDetailDialog from "../assets/components/ExportAssetDetailDialog.vue";
...
export default {
  ...
  methods: {
    ...
    deleteDialog() {
      AssetManagementService.archiveOrDeleteAsset(this.assetUUID, "delete")
        .then((response) => {
    ...
    archiveDialog() {
      AssetManagementService.archiveOrDeleteAsset(this.assetUUID, "archive")
        .then((response) => {
          if (response.status === 200) {
            this.loadTable(this.sdtData);
            this.showToast("Asset Archived Successfully", "success");
            this.archiveInspectionDialog = false;
          }
        })
        .catch((error) => {
    ...
    getActionsOption(val) {
      ...
      } else if (val.value == "Create Issue") {
        this.newIssue = true;
        this.assetObj = val.row;
        this.assetObj.assetFlag = true;
        this.indexKey++;
      } else if (val.value == "Create WO") {
        this.$router.push({
      ...
      } else if (val.value == "Create SR") {
        this.addServiceDialog = true;
        this.assetObj = val.row;
        this.assetObj.assetNameDisable = true;
        this.indexKey++;
    ...
    catchDeparture: function (data) {
      ...
      if (data.status == 1 || data.status == 3) {
        ...
        if (data.message.checkbox !== undefined) {
          let asset = data.message;
          if (asset.checkbox.checked) {
            this.opt.push(asset);
          } else {
            let index = this.opt.findIndex((a) => a.uuid === asset.uuid);
            if (index > -1) {
              this.opt.splice(index, 1);
            }
          }
        } else {
          let assets = data.message;
          assets.map((asset) => {
            if (asset.checkbox.checked) {
              this.opt.push(asset);
            } else {
  ...
  watch: {
    assetOption: function (newVal, oldVal) {
      if (newVal == "asset-group") {
        ...
        let allConfigs = JSON.parse(this.orgConfigsResponse.data.tableConfigs);
        allConfigs = allConfigs.filter(
          (data) => data.pageName === "archivedAssets"
        )[0];

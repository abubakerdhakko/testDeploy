<template>
  <div class="shark-data-table pt-32 pb-80">
    <div style="margin-bottom: 40px">
      ...
      ><br />
      <span class="subtitle"
        >View all the information related to assets categories</span
      >
    </div>
    ...
    <div v-if="assetOption === 'archivedAssets'">
      <ArchivedAssetCate :configs="archivedAssetsConfigs" />
    </div>
...
import ArchivedAssetCate from "./components/ArchivedAssetsCate.vue";
...
export default {
  ...
  methods: {
    ...
    deleteDialog() {
      this.deleteInspectionDialog = false;
      this.loader= true
      AssetManagementService.DeleteAssetCate(this.assetUUID, "delete") 
        .then((response) => {
    ...
    archiveDialog() {
      this.loader= false
      this.deleteInspectionDialog = false;
      AssetManagementService.archiveOrDeleteAssetCate(this.assetUUID, "archived")
        .then((response) => {
          if (response.status === 200) {
            this.loadTable(this.sdtData);
    ...
    getActionsOption(val) {
      ...
      } else if (val.value == "Add Asset") {
        this.$router.push({
          name: "create-asset-by-category",
          params: {
            AddByCateFlag: true,
            assetCateID: val.row.uuid,
            slug: 'Add-AssetBy-Cate'
          },

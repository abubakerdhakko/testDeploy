<template>
  <div>
    ...
    />
    <DeleteDialog
      v-if="deleteAchivedCate"
      @close="closeDialog"
      @deleteData="deleteArchivedDialog"
      @canceled="closeDialog"
    />
...
export default {
  name: "ArchivedCateTemplate",

  components: { RestoreDialog, loader,DeleteDialog },

  data() {
    return {
      loaderFlag: false,
      restoreAssetDialog: false,
      deleteAchivedCate:false,
      user: "",
      username: "",
      isLoadingPage: false,
      isLoading: false,
  ...
  methods: {
    closeDialog() {
      ...
      this.deleteAchivedCate =false
    },
 
    restoreArchived() {
      this.restoreAssetDialog = false;
      this.loaderFlag = true;
      AssetManagementService.deleteArchivedAssetCate(
        this.assetId,
        "restore"
      )
        .then((response) => {
          if (response.status === 200) {
            ...
            this.showToast("Asset Category Restored Successfully", "success");
            this.restoreAssetDialog = false;
            this.loaderFlag = true;
          }
        })
    ...
    deleteArchivedDialog() {
      this.deleteAchivedCate = false;
      this.loaderFlag = true;
      AssetManagementService.archiveOrDeleteAssetCate(
        this.assetId,
        "Delete"
      )
        .then((response) => {
          if (response.status === 200) {
            this.loadTable(this.sdtData);
            this.showToast("Asset Category Deleted Successfully", "success");
            this.deleteAchivedCate = false;
            this.loaderFlag = true;
          }
        })
    ...
    getPaginatedArchivedAssets(
      ...
      AssetManagementService.getPaginatedAssetCate(requestObj)
        .then((response) => {
          if (response.status === 200) {

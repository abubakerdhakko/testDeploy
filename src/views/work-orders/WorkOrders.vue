<template>
  <div class="shark-data-table">
    <div class="pt-32">
      <span class="head-title">Work Orders</span><br />
      <span class="subtitle"
        >View all the information related to work orders</span
      >
    </div>
    <div class="buttons-row">
      <SToggle
        ...
        @selected="selectWorkOrderType"
      />
      <SButton
        v-if="selectWorkOrder === 'work-order'"
        title="Create Work Order"
        pill
        outline
        small
        @click="openCreateWorkOrder()"
        v-width="200"
      ></SButton>
    </div>
    <div v-if="selectWorkOrder === 'work-order'">
      <Sdt
    ...
    <div v-if="selectWorkOrder === 'archived'">
        <ArchivedWorkOrder :configs="archivedConfigs"/>
    </div>
    <DeleteDialog
...
import ArchivedWorkOrder from "./ArchivedWorkOrder.vue"
...
export default {
  ...
  data() {
    return {
      ...
      parentSystemSettings: {
        ...
        selectType: "none",
        moreOptions: [],
        actionsOptions: ["View", "Edit", "Archive", "Delete"],
        columnIcons: [],
        pillConfig: [
  ...
  methods: {
    selectWorkOrderType(obj){
        this.selectWorkOrder = obj.value
    },

    closeDialog() {
      this.deleteWorkOrderDialog = false;
      this.archiveWorkOrderDialog = false;
    },
    ...
    archiveDialog() {
      this.archiveWorkOrderDialog = false;
      this.deleteWorkOrder(this.workOrderID, "Archive");
    },
        ...
        .then((response) => {
          if (response.status === 200) {
            if (type === "Delete") {
              ...
              this.showToast("Archived Work Order Successfully", "Success");
              this.archiveWorkOrderDialog = false;
              this.loadTable(this.sdtData);
            }
          }
    ...
    getActionsOption(val) {
      ...
      if (val.value == "Archive") {
        if (val.row.status !== "In Progress") {
          ...
          this.archiveWorkOrderDialog = true;
        } else {
          this.showToast(
            "You are unable to archive an in-progress workorder",
            "warning"
          );
        }
      }
